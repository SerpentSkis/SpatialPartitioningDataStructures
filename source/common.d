...
unittest
{
    ...
    const auto aabb = boundingBox(points);
...
unittest
{
    ...
    assert(points[0].data == [1, 2]);
    assert(points[1].data == [2, 3]);
    assert(points[2].data == [3, 1]);
...
unittest
{
    ...
    const auto leftHalf = points[0 .. $ - rightHalf.length];
...
unittest
{
    ...
    auto gPoints = getGaussianPoints!3(10_000);
    //no guarantees here...
